...
const styles = theme => ({
    ...
    formControl: {
        margin: 8, // Manually defined spacing
        minWidth: 240,
        maxWidth: 240
    },
    title: {
...
class Home extends Component {
    ...
    componentDidMount() {
        ...
        this.fetchData("genres", "genresList");
        this.fetchData("artists", "artistsList");
    }

    fetchMovies(status, stateField) {
        fetch(this.props.baseUrl + `movies?status=${status}`)
            .then(response => response.json())
            .then(data => this.setState({ [stateField]: data.movies }))
            .catch(error => console.error("Error fetching movies:", error));
    }

    fetchData(endpoint, stateField) {
        fetch(this.props.baseUrl + endpoint)
            .then(response => response.json())
            .then(data => this.setState({ [stateField]: data[endpoint] }))
